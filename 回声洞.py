...
class EchoCave:
    def __init__(self, root):
        ...
        # 创建主窗口左下角按钮（点击后打开回声洞窗口）
        self.echo_cave_btn = tk.Button(
            root,
            text="回声洞",
            font=("SimHei", 12),
            command=self.open_echo_cave_window
    
        )
        # 固定按钮在左下角
        self.echo_cave_btn.place(relx=0.05, rely=0.9, anchor="sw")  

        # 用于存储回声洞独立窗口
        self.echo_window = None  

    def open_echo_cave_window(self):
        """打开独立的回声洞交互窗口"""
        if self.echo_window and self.echo_window.winfo_exists():
            self.echo_window.lift()  # 窗口已存在则激活
            return

        self.echo_window = tk.Toplevel(self.root)
        self.echo_window.title("回声洞")
        self.echo_window.geometry("500x300")  

        # 构建回声洞交互界面（原逻辑迁移）
        self.build_echo_interface(self.echo_window)
    ...
    def show_next_char(self):
        ...
        if self.current_char_index < len(self.current_text):
            ...
            # 30毫秒后显示下一个字
            self.echo_window.after(30, self.show_next_char)
        else:
            # 显示完可再次点击
            self.is_showing = False
