...
def setting():
    try:
        ...
        messagebox.showerror("错误", f"执行失败: {str(e)}")
# [alpha]一个用于下载1.8.9的测试函数，下面的路径自己改咯
def alpha():
    start.download_minecraft_version(version_name = '1.8.9' , minecraft_dir = r'C:/dml')


# 主程序
# 设置任务名
ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("dml.dml.0.2")
# 主界面
root = tk.Tk()
root.title('DML 0.1')
